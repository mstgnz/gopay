...
func TestNewProvider(t *testing.T) {
	...
	_, ok := p.(*AkbankProvider)
	if !ok {
		t.Fatal("NewProvider() did not return *AkbankProvider")
	}
}
...
func TestInitialize(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !tt.wantErr {
				akbank := p.(*AkbankProvider)
				if akbank.merchantSafeId != tt.config["merchantSafeId"] {
					t.Errorf("merchantSafeId = %v, want %v", akbank.merchantSafeId, tt.config["merchantSafeId"])
				}
				if akbank.terminalSafeId != tt.config["terminalSafeId"] {
					t.Errorf("terminalSafeId = %v, want %v", akbank.terminalSafeId, tt.config["terminalSafeId"])
				}
				if akbank.secretKey != tt.config["secretKey"] {
					t.Errorf("secretKey = %v, want %v", akbank.secretKey, tt.config["secretKey"])
				}
			}
		})
	}
